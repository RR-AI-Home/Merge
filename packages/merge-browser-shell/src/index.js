export function bootMergeBrowserApp({ themeRoot, saveKey, session }) {
  const log = [];
  let theme;
  let save;
  ...
  async function readThemeJson(fileName) {
    const response = await fetch(new URL(fileName, themeRoot));
    if (!response.ok) {
  ...
  function loadSave() {
    ...
    if (!stored) {
      return session.createInitialSave(theme);
    }
  ...
  function persist() {
    localStorage.setItem(saveKey, JSON.stringify(session.serializeSave(save)));
  }
  ...
  function renderStatus() {
    ...
    elements.district.textContent = session.getCurrentDistrict(save, theme).title;
    ...
    const progress = session.getChapterProgress(save, theme);
    ...
    const eventRail = session.getEventRail(save, theme);
    ...
    const goal = session.getSessionGoal(save, theme);
  ...
  function renderBoard() {
    ...
    for (const cell of save.board.cells) {
      ...
      const described = session.describeBoardCell(cell, theme);
      button.type = 'button';
  ...
  function renderOrders() {
    const openOrders = session.getOpenOrders(save, theme);
    ...
    for (const order of openOrders) {
      ...
      button.addEventListener('click', () => {
        applyResult(session.completeOrderFromBoard(save, theme, order.id));
      });
  ...
  function handleCellClick(cell) {
    ...
    const result = cell.item
      ? session.mergeBoardCells(save, theme, { from: selectedCell, to: target })
      : session.moveItem(save, selectedCell, target);
  ...
  function bindActions() {
    elements.producerButton.addEventListener('click', () => {
      applyResult(session.tapPrimaryProducer(save, theme));
      playFeedback('produce');
    });

    elements.dailyButton.addEventListener('click', () => {
      applyResult(session.claimDailyLogin(save, {
        calendarDay: new Date().toISOString().slice(0, 10)
      }));
    });

    elements.resetButton.addEventListener('click', () => {
      save = session.createInitialSave(theme);
      selectedCell = null;
  ...
  async function boot() {
    ...
    save = loadSave();
    bindActions();
    addLog(`${theme.config.displayName} prototype ready.`);
    render();
  }
